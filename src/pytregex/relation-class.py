...
class HEADS(Relation):
    ...
    @classmethod
    def searchNodeIterator(cls, t: "Tree", headFinder: Optional["HeadFinder"] = None) -> Generator["Tree", None, None]:
        if headFinder is None:
            headFinder = cls.hf
        parent_ = t.parent
...
class IMMEDIATELY_PRECEDES(Relation):
    ...
    def searchNodeIterator(cls, t: "Tree") -> Generator["Tree", None, None]:
        ...
        for i,kid in enumerate(parent_.children):
            if kid is current:
                break
        next = parent_.children[i+1]# Use i+1 won't cause IndexError because current is not the last child
        while True:
            yield next
            if next.isLeaf():
                break
...
class UNBROKEN_CATEGORY_DOMINATES(Relation):
    ...
    @classmethod
    def satisfies(cls, t1: "Tree", t2: "Tree", rel_arg: List["Tree"]) -> bool:
        #TODO passing in rel_arg is expansive, may be passing in node_descriptions is better?
        for kid in t1.children:
            if kid is t2:
                return True
            else:
                if any(kid is node for node in rel_arg) and UNBROKEN_CATEGORY_DOMINATES.satisfies(kid, t2, rel_arg):
                    return True
        return False

    @classmethod
    def searchNodeIterator(cls, t: "Tree", rel_arg: List["Tree"]) -> Generator["Tree", None, None]:
        ...
        for kid in t.children:
            ...
            if any(kid is node for node in rel_arg):
                for descendant in cls.searchNodeIterator(kid, rel_arg):
                    yield descendant
...
class UNBROKEN_CATEGORY_IS_DOMINATED_BY(Relation):
    ...
    def satisfies(
        ...
        return UNBROKEN_CATEGORY_DOMINATES.satisfies(t2, t1, rel_arg)
    ...
    def searchNodeIterator(cls, t: "Tree", rel_arg: List["Tree"]) -> Generator["Tree", None, None]:
        ...
        while True:
            if parent_ is None:
                break
            yield parent_
            if not any(parent_ is node for node in rel_arg):
                break
            parent_ = parent_.parent
...
class UNBROKEN_CATEGORY_PRECEDES(Relation):
    ...
    def satisfies(cls, t1: "Tree", t2: "Tree", rel_arg: List["Tree"]) -> bool:
        ...
        if immediate_follower is t2:
            ...
            if any(immediate_follower is node for node in rel_arg) and UNBROKEN_CATEGORY_PRECEDES.satisfies(
                immediate_follower, t2, rel_arg
            ):
                return True
        return False
    ...
    def searchNodeIterator(cls, t: "Tree", rel_arg: List["Tree"]) -> Generator["Tree", None, None]:
        for immediate_follower in IMMEDIATELY_PRECEDES.searchNodeIterator(t):
            ...
            if any(immediate_follower is node for node in rel_arg):
                for after_follower in cls.searchNodeIterator(immediate_follower, rel_arg):
...
class UNBROKEN_CATEGORY_FOLLOWS(Relation):
    ...
    @classmethod
    def satisfies(cls, t1: "Tree", t2: "Tree", rel_arg: List["Tree"]) -> bool:
        return UNBROKEN_CATEGORY_PRECEDES.satisfies(t2, t1, rel_arg)

    @classmethod
    def searchNodeIterator(cls, t: "Tree", rel_arg: List["Tree"]) -> Generator["Tree", None, None]:
        for immediate_precedent in IMMEDIATELY_FOLLOWS.searchNodeIterator(t):
            yield immediate_precedent
            if any(immediate_precedent is node for node in rel_arg):
                for before_precedent in cls.searchNodeIterator(immediate_precedent, rel_arg):
